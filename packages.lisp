(defpackage #:environment
  ...
  (:export
   #:environment #:alist-environment #:hash-table-environment #:meta-environment #:dynamic-environment #:reverse-environment #:dynamic-environment
   #:env-mapping #:env-dynamic-frames #:env-reverse-mapping
   ...
   #:bind #:name-bound-p #:do-unbind #:unbind #:lookup-value #:set-value #:env-alist #:map-environment #:copy-environment
   ...
   #:with-fresh-dynamic-frame #:find-dynamic-frame #:name-dynamic-p #:evaluate-dynamic #:dynamic #:set-dynamic #:allocate-dynamic #:undo-dynamic #:with-dynamic-frame-bindings #:allocate-dynamic-binding))

(defpackage #:allocation-pool
  (:nicknames :allocpool)
  (:use :common-lisp :alexandria :iterate :pergamum :environment)
  (:export
   #:pool-environment #:dynamic-pool-environment
   #:allocation-condition #:allocation-error #:simple-allocation-error
   #:env-freelist
   #:make-dynamic-pool
   #:allocate #:release #:with-pool-allocation #:with-pool-subset #:pool-allocate-dynamic))
